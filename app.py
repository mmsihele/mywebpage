...
local_css("style/style.css")  # Ensure this path is correct
...
try:
    ...
    st.warning("Contact form image not found. Using placeholder.")

try:
    img_github = Image.open("462567014_1092285125367772_5253116121943760911_n.jpg")
except FileNotFoundError:
    img_github = None
    st.warning("GitHub image not found. Using placeholder.")

# Header section
with st.container():
    st.subheader("Hi, I am Mishele Ann")
    st.title("Let's Explore How Computer Engineering Can Help!")
    st.write(
        """
...
with st.container():
    ...
    with image_column:
        if img_contact_form:
            st.image(img_contact_form, caption="Computer Engineering Student: The Selcouth of Taking Risks")
        else:
